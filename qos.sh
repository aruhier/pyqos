...
case "$1" in
    start)
        ...
        echo "#---interactive - id 100 - rate $(( $UPLINK * 1 / 10 )) kbit ceil $(( $UPLINK * 80 / 100 )) kbit"
        $TC class add dev $PUBLIC_IF parent 1:1 classid 1:100 htb \
            rate $(( $UPLINK * 10 / 100 ))kbit ceil $(( $UPLINK * 80 / 100 ))kbit \
            burst 5k prio 1
        ...
        echo "#---tcp acks - id 200 - rate $(( $UPLINK * 0.5 )) kbit ceil $(( $UPLINK )) kbit"
        $TC class add dev $PUBLIC_IF parent 1:1 classid 1:200 htb \
            rate $(( $UPLINK * 0.7 ))kbit ceil $(( $UPLINK ))kbit burst 15k prio 2
        ...
        $TC qdisc add dev $PUBLIC_IF parent 1:999 handle 1999: sfq perturb 10 limit 32
        echo "#--- ~ filtre default"
